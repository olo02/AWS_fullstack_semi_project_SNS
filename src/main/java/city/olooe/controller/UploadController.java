...
public class UploadController {
    ...
    public List<AttachFileDTO> uploadAjax(MultipartFile[] files) {
        ...
        for(MultipartFile m : files) {
            ...
            dto.setImage(image);
            dto.setAname(m.getOriginalFilename());
            list.add(dto);
        }
        return list;
    }
...
//        log.info(dto);
        String s = PATH + "/" + dto.getPath() + "/" + (thumb != null && thumb ? "s_" : "") + dto.getUuid() + "_" + dto.getAname();
...
//            e.printStackTrace();
    ...
    public String deleteFile(AttachFileDTO dto) {
        String s = PATH + "/" + dto.getPath() + "/" + dto.getUuid() + "_" + dto.getAname();
        File file = new File(s);
        log.warn(s);
        ...
        if (dto.isImage()) {
            s = PATH + "/" + dto.getPath() + "/s_" + dto.getUuid() + "_" + dto.getAname();
            file = new File(s);
            file.delete();
        }
        return dto.getUuid();
    ...
    public ResponseEntity<byte[]> download(AttachFileDTO dto, Boolean thumb) {
        ...
        try {
            ...
            headers.add("Content-Disposition","attachment; filename=" + new String(dto.getAname().getBytes("utf-8"), "iso-8859-1"));
            result = new ResponseEntity<>(FileCopyUtils.copyToByteArray(file), headers, HttpStatus.OK);
        } catch (IOException e) {
            e.printStackTrace();
        }
